...
class Settings:
    ...
    switch_data_dir: Path = Path(
        os.getenv("DEADMAN_SWITCH_DATA_DIR", str(_tenant_default_path("deadman_switch")))
    )
    # 客服工单数据
    support_data_dir: Path = Path(
        os.getenv("DEADMAN_SUPPORT_DATA_DIR", str(_tenant_default_path("support")))
    )
    # Onboarding 向导数据
    onboarding_data_dir: Path = Path(
        os.getenv("DEADMAN_ONBOARDING_DATA_DIR", str(_tenant_default_path("onboarding")))
    )
    ...
    license_secret: str = os.getenv("DEADMAN_LICENSE_SECRET", "")
    # 授权码 token（或挂载文件 /app/license.lic）
    license_key: str = os.getenv("DEADMAN_LICENSE_KEY", "")
