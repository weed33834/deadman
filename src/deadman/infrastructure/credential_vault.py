...
# 可选 AES-256-GCM 依赖（统一经 utils.crypto 使用，缺失时 _encrypt/_decrypt 拒绝降级）
try:
    import secrets

    from ..utils.crypto import decrypt_bytes, encrypt_bytes  # type: ignore
...
def _encrypt(plaintext: str, master_key: bytes) -> str:
    ...
    return base64.b64encode(encrypt_bytes(plaintext.encode("utf-8"), master_key)).decode("ascii")
...
def _decrypt(encrypted: str, master_key: bytes) -> str:
    ...
    return decrypt_bytes(raw, master_key).decode("utf-8")
