...
def _get_switch_passphrase(user_id: str, tenant_id: str | None = None) -> bytes:
    """从全局 secret 派生 deadman_switch 专用口令
    ...
    Step 4 per-tenant：multi 模式用 tenant_id 派生（租户间互不可解密），
    single 模式沿用 per-user 派生（与旧数据零迁移兼容）。
    """
    ...
    if tenant_id:
        # per-tenant 派生：HMAC(secret, "tenant:"+tenant_id+":"+label)
        data = ("tenant:" + tenant_id + ":" + _PASSPHRASE_LABEL).encode("utf-8")
    else:
        # per-user 派生（single 模式向后兼容）
        data = (_PASSPHRASE_LABEL + ":" + user_id).encode("utf-8")
    return hmac.new(global_secret.encode("utf-8"), data, hashlib.sha256).digest()
...
class SwitchStore:
    ...
    def __init__(self, data_dir: Path | None = None) -> None:
        if data_dir is None:
            ...
            if env_dir:
                ...
                data_dir = resolve_tenant_path("deadman_switch")
    ...
    def _decrypt_record(self, envelope: dict[str, Any], user_id: str) -> SwitchRecord | None:
        """解密 envelope dict -> SwitchRecord（per-tenant 优先，兼容旧 per-user）"""
        try:
            try:
                ...
                plaintext = _decrypt(envelope, legacy)
