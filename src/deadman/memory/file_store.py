...
def _default_memory_dir() -> Path:
    ...
    return resolve_tenant_path("memory")


DEFAULT_MEMORY_DIR: Path | None = None
...
def _atomic_write(path: Path, content: str) -> None:
    """原子写入文本（委托 utils.jsonio，避免各处重复 tmp+replace 样板）。"""
...
class FileMemoryStore:
    ...
    def __init__(self, memory_dir: Path | None = None) -> None:
        ...
        """
        self.memory_dir: Path = memory_dir if memory_dir is not None else _default_memory_dir()
