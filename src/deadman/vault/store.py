...
class VaultStore:
    ...
    def __init__(self, data_dir: Path | None = None) -> None:
        ...
        Args:
            data_dir: 数据根目录，默认 ~/.deadman/vault/（多租户时按租户路由）
        """
        if data_dir is None:
            from ..infrastructure.multi_tenant import resolve_tenant_path

            data_dir = resolve_tenant_path("vault")
    ...
    def _derive_key(self, user_id: str, password: str, tenant_id: str | None = None) -> bytes:
        """从用户 ID + 主密码派生加密密钥（PBKDF2-HMAC-SHA256）

        Args:
            user_id: 用户 ID（作为 salt 的一部分）
            password: 主密码（来自环境变量或上游认证系统）
            tenant_id: 租户 ID（multi 模式传入；single 模式为 None 保持旧派生）

        Returns:
            32 字节派生密钥

        Step 4 per-tenant：multi 模式把 tenant_id 纳入 salt，租户间即使
        同一 user_id 也派生不同密钥（数据隔离）。single 模式 salt 不变，
        旧数据零迁移可读。
        """
        ...
        salt_part = (tenant_id + "::") if tenant_id else ""
        salt = (salt_part + user_id + "::deadman-vault").encode("utf-8")
    ...
    def _do_deliver(self, item: VaultItem, beneficiary_id: str) -> dict[str, Any]:
        ...
        Step 4 per-tenant：先试 per-tenant 密钥；失败时 fallback 旧 per-user
        派生密钥（迁移期兼容，成功即返回明文，后续写入会重加密为 per-tenant）。
        """
        owner_id = item.owner_user_id
        master = self._get_master_password()
        key = self._derive_key(owner_id, master)
        plaintext = self._decrypt(item.content_encrypted, key)
        if not plaintext:
            # per-tenant 密钥解不开 → 旧 per-user 派生（single 模式密钥一致，无额外开销）
            legacy_key = self._derive_key(owner_id, master, tenant_id="")
            if legacy_key != key:
                plaintext = self._decrypt(item.content_encrypted, legacy_key)
        if not plaintext:
            return {
                "delivered": False,
                "content": None,
