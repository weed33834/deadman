...
from .routes import canvas as _canvas_routes
from .routes import chat_extras as _chat_extras_routes
from .routes import customs as _customs_routes
...
from .routes import org_pages as _org_pages_routes
from .routes import orgs as _orgs_routes
...
async def lifespan(app: FastAPI):
    ...
    # To B 授权码（B2B-IMPLEMENTATION Step 7.1）：启动即上报状态；过期进入只读
    try:
        ...
        logger.warning("授权状态检查异常（忽略，不影响启动）: %s", exc)
...
app.include_router(_canvas_routes.router)
app.include_router(_customs_routes.router)
app.include_router(_kinship_routes.router)
...
app.include_router(_org_export_routes.router)
...
async def auth_login(req: LoginRequest):
    ...
    try:
        ...
        if len(active_orgs) == 1:
            token = jwt_mgr.issue(
                user,
                tenant_id=active_orgs[0].org_id,
                org_role=active_orgs[0].org_role,
            )
    except Exception:
        logger.warning("login: 机构上下文自动带入失败，按未绑定机构签发", exc_info=True)
    expires_at = _token_expiry_iso(jwt_mgr, token)
    return {
...
@app.get("/api/license/status", tags=["ops"])
async def license_status_endpoint():
    """To B 授权状态（trial / licensed / expired；过期时 readonly=true）"""
    from ..billing.license import license_status
