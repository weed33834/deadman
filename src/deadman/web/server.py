...
⚠️ DEPRECATED（B2B-IMPLEMENTATION Step 8.1）：本模块为旧 stdlib http.server
实现，已废弃。统一入口为 uvicorn deadman.web.app:app（FastAPI 版，含机构
工作台 / To B 多租户 / 授权码 / 数据导出）。原 cli 入口 deadman-web-server-legacy
已从 pyproject 移除，本模块仅保留供内部调试引用，M6 后删除。
