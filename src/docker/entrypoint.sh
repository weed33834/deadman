...
#   web-server  (默认) 启动 Web Server（机构工作台 + 对话界面 + 运维 API，端口 8002）
#   mcp-server         启动 MCP Server 的 HTTP 传输模式
#   eval               运行自动化评估套件
#   run "<输入>"        运行单次对话
#
# 用法：
#   docker run deadman                          # 默认 web-server
#   docker run deadman web-server               # 显式指定
#   docker run deadman mcp-server               # MCP（端口 8000）
