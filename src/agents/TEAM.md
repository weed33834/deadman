...
8 个并列智能体之下有一层共享的支撑设施，不改变 agent.md 驱动的核心架构，只为智能体提供更强的能力：
...
### 7. deep-researcher（深度研究员）

**定位**：多来源深度调研与交叉验证，面向系统性 / 对比性 / 核验性诉求。

**职责**：
- 问题拆解为多个聚焦子问题
- 迭代检索多来源（deep_research 工具 + WebSearch）
- 多源交叉验证一致性，标记低可信度 / 需官方核实项
- 输出带引用的结构化研究报告（概述 + 分点结论 + 来源清单）

**转介触发**：
- 单点、即时的政策查询 → 转介 policy-researcher
- 需要通用流程引导 → 转介 death-aftercare
- 涉及法律争议 / 财务税务的深度调研 → 可与 legal-advisor / financial-analyst 协作

### 8. data-analyst（数据分析员）

**定位**：表格数据描述性统计与解读（客户/案件/财务清单的汇总、分布、异常）。

**职责**：
- 对数据做列类型 / 行数 / 数值列均值 / 中位数 / 标准差 / 极值 / 缺失统计
- 识别异常值、缺失、分布倾斜
- 用自然语言解读统计结果并给可操作提示

**转介触发**：
- 需要把分析结论落地到业务决策（续费/预警/定价）→ 转介 financial-analyst
- 需要通用流程引导 → 转介 death-aftercare
...
| 类型 | 说明 | 实现 |
|------|------|------|
| **内部转介** | 在本平台 8 个并列智能体之间转介 | LangGraph conditional edge / TRAE 用户切换 / Coze 工作流路由 |
| **外部转介（A2A）** | 转介到别家厂商的智能体（如别家律师 agent） | [A2A Protocol](../a2a/A2A-Protocol.md)，需用户数据共享同意 |
...
外部转介额外约束（见 [A2A-Protocol.md](../a2a/A2A-Protocol.md)）：
