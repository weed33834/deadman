...
# 模式：  docker run deadman:latest web-server   # 默认（To B 交付）
#         docker run -p 8002:8002 deadman:latest web-server
#         docker run deadman:latest mcp-server
...
# B2B-IMPLEMENTATION Step 8.3：版本从 pyproject 单源注入（构建时传 DEADMAN_VERSION）
ARG DEADMAN_VERSION=6.1.0
...
# 镜像元数据（OCI 标准）
# B2B-IMPLEMENTATION Step 8.3：license 从 pyproject 单源（Apache-2.0）
LABEL org.opencontainers.image.title="deadman" \
      org.opencontainers.image.description="身后事 To B 多租户机构平台 Web Server（含机构工作台 / 授权码 / 数据导出）" \
...
# 默认启动 Web Server（B2B-IMPLEMENTATION Step 8.3：To B 交付默认形态）
# 可通过 docker run ... <mode> 覆盖：web-server | mcp-server | eval | run
CMD ["web-server"]
