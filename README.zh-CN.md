...
> **deadman** —— 生前准备 + 身后办理 AI 引导员平台。**对话优先 / 傻瓜式操作**：直接说话或输入，Agent 即可引导你一步步办好，无需点页面。支持**民俗/规则定制**与**亲属图谱可视化**。不绑定厂商，适用于任意 agent 平台。

> **机构版（To B）**：deadman 以**多租户机构平台**形态交付——殡仪 / 保险 / 律所 / 身后服务机构可通过**机构工作台**（`/org`）管理客户、案件、审计日志、知识库与团队角色；支持**授权码授权**（30 天试用、过期只读）与**数据导出**（CSV/JSON/zip）。租户数据经 `resolve_tenant_path()` 完全隔离。
