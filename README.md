...
- 身后事规划完整度评分（5 维度）
- 数字遗产清单（结构化资产登记 + 继承人指派 + 移交 / 注销可执行方案，对标 Cipherwill / GoodTrust）
- 哀伤陪伴（面向丧亲者的共情陪伴 + 危机识别护栏，对标 Akeeva / SafeKeep）
- 思维意识识别（意图分类 + 安全状态感知，路由到正确能力；L0 危机强制干预）
- Web 响应式适配移动端（侧栏抽屉 + 底部安全区避让，手机可用）
