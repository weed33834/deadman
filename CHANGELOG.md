...
- `intent.py`：关键词打分意图分类（遗嘱/办事/哀伤/数字遗产/死人开关/纪念/知识查询/通用），可选 LLM 兜底判定
- `recognizer.py`：意图 + 安全状态（`grief.detect_crisis`）联合感知，产出推荐能力路由；**L0 危机一律覆盖意图、强制走哀伤陪伴 + 危机干预**（安全优先）
- 接入 ReAct 工具表（`awareness` 工具），智能体分发动作前可自省用户意图与状态，不为死代码
- 11 个测试覆盖意图分类、LLM 兜底、危机优先路由、能力映射

### 改进：移动端界面适配（iOS 安全区）
- web 主应用响应式（Phase 16C）已具备，本轮补 `viewport-fit=cover` + `env(safe-area-inset-*)`：汉堡按钮 / 顶栏 / 对话区 / 输入区 / 返回顶部按钮避开刘海与底部 Home 指示条
